...
namespace Reflectable
{
    ...
    public sealed class ReflectableGameController : MonoBehaviour
    {
        ...
        [Header("UI")][SerializeField] Text hpText, scoreText, gemsText, turnText, comboText, levelText, characterText, gameOverText; [SerializeField] GameObject pausePanel, gameOverPanel, upgradePanel, characterPanel, stageIntro; [SerializeField] Button powerButton, ricochetButton, extraBallButton, maxHpButton, characterButton;
        ...
        public void StartNewGame(){StopAllCoroutines();ClearBlocks();turn=0;hp=maxHp=100;score=gems=exp=skillPoints=power=ricochet=extraBall=maxHpUpgrade=combo=maxCombo=destroyed=ricochets=0;level=rank=1;character="MIMI";gachaCost=2;gameOver=false;stageCleared=false;aiming=false;if(gameOverPanel)gameOverPanel.SetActive(false);stage=ReflectableStageSession.ResolveSelectedStage();SpawnStartingBlocks(stage);StartCoroutine(StageIntroRoutine());}
